...
## Get the AF DRU-Level pubs
#DRU
get_internet = driver.find_element_by_xpath('//*[@id="mvcContainer-449"]/div[2]/div[1]/div[1]/ul/li[6]/a')
get_internet.click()
time.sleep(2)
...
for m in range(1,4):
    ...
    ## select the DRU
    get_internet = driver.find_element_by_xpath('//*[@id="cat-3"]/div/ul/li[' + str(m) + ']/a')
...
df.to_csv('logs/AF_epubs_dru.csv', index=False)

### Save the links
#with open('links/AF_epubs_dru.txt', 'w') as f:
#    for item in hrefs:
#        f.write("%s\n" % item)
## close the session
driver.quit()
