...
## Get the Bases-Level pubs
#Bases
get_internet = driver.find_element_by_xpath('//*[@id="mvcContainer-449"]/div[2]/div[1]/div[1]/ul/li[9]/a')
get_internet.click()
time.sleep(3)
...
## scan through the different Bases
hrefs = []
titles = []
for m in range(1,101+1):
    ...
    ## select the Base
    get_internet = driver.find_element_by_xpath('//*[@id="cat-6"]/div/ul/li[' + str(m) + ']/a')
    print('m = %i, navigating to: %s' %(m, get_internet.text))
    try:
        get_internet.click()
        time.sleep(6)
...
df.to_csv('logs/AF_epubs_bases.csv', index=False)

### Save the links
#with open('links/AF_epubs_bases.txt', 'w') as f:
#    for item in hrefs:
#        f.write("%s\n" % item)
