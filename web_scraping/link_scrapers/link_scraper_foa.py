...
## Get the AF FOA-Level pubs
#FOA
get_internet = driver.find_element_by_xpath('//*[@id="mvcContainer-449"]/div[2]/div[1]/div[1]/ul/li[7]/a')
get_internet.click()
time.sleep(5)
...
for m in range(1,27):
    ...
    ## select the FOA
    get_internet = driver.find_element_by_xpath('//*[@id="cat-4"]/div/ul/li[' + str(m) + ']/a')
...
df.to_csv('logs/AF_epubs_foa.csv', index=False)


### Save the links
#with open('links/AF_epubs_foa.txt', 'w') as f:
#    for item in hrefs:
#        f.write("%s\n" % item)
## close the session
driver.quit()
