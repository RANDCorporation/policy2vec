...
## Get the Units-Level pubs
#Units
get_internet = driver.find_element_by_xpath('//*[@id="mvcContainer-449"]/div[2]/div[1]/div[1]/ul/li[10]/a')
get_internet.click()
time.sleep(5)
...
hrefs = []
titles = []
for m in range(1,160+1):
    ## navigate to the top-level page
    driver.get(top_level_url)

    ## select the Unit
    get_internet = driver.find_element_by_xpath('//*[@id="cat-7"]/div/ul/li[' + str(m) + ']/a')
    print('m = ', m, 'navigating to: ' + get_internet.text)
    try:
        get_internet.click()
        time.sleep(10)
...
df.to_csv('logs/AF_epubs_units.csv', index=False)
